...
export default function LocationCard({ location, onSelectItem }: Props) {
  ...
  const isClosed = !location.isOpen || location.meals.length === 0;
        ...
        {location.meals.length > 1 && (
          <div style={{ display: "flex", gap: "0.35rem", marginTop: "0.6rem", overflowX: "auto", scrollbarWidth: "none" }}>
            {location.meals.map((meal, idx) => {
              ...
              return (
                // key by index rather than name — names could theoretically repeat
                <button key={idx} onClick={() => setActiveMeal(meal.name)} style={{
